...
# Import tool registry update function
from agent.tools.tool_registry_update import register_tools
...
async def run_agent(
    ...
    # Register all new tools from v7 enhancements
    if config.ENV_MODE == 'local':
        logger.info("🔧 Registering enhanced tools for local environment")
        register_tools(thread_manager.tool_registry, project_id, thread_id, thread_manager)
    else:
        logger.info("ℹ️ Skipping enhanced tools registration in non-local environment")
