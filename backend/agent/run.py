...
#     # Create a test project for this thread
#     try:
#         account_id = account_result.data[0]['account_id']
#         print(f"\n👤 Using account: {account_id}")

#         # Check if we already have a test project
#         project_result = await client.table('projects').select('*').eq('account_id', account_id).eq('name', 'Test Project').execute()
...
#                 'name': 'Test Project',
#                 'description': 'A test project for the agent',
#                 'sandbox': {
#                     'id': 'test-sandbox',
#                     'status': 'running'
#                 }
